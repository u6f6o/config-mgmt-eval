...
docker build -t zookeeper -f ConsulFile . 

# 1st zookeeper server node
nohup docker run --name zksrv1 -h zksrv1 -p 8500:8500 zookeeper -server -ui-dir /ui -config-dir /config -bootstrap-expect 3 2>&1 | sed -u 's/^/zk-srv1] /' >> /var/tmp/zookeeper_cluster.log &
sleep 2
JOIN_IP="$(docker inspect -f '{{.NetworkSettings.IPAddress}}' zksrv1)"

# 2nd zookeeper server node
nohup docker run --name zksrv2 -h zksrv2 zookeeper -server -join $JOIN_IP 2>&1 | sed -u 's/^/[zk-srv2] /' >> /var/tmp/zookeeper_cluster.log &
sleep 2

# 3rd zookeeper server node
nohup docker run --name zksrv3 -h zksrv3 zookeeper -server -join $JOIN_IP 2>&1 | sed -u 's/^/[zk-srv3] /' >> /var/tmp/zookeeper_cluster.log &
sleep 2
